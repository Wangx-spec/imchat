...
class HybridRetriever:
    ...
    def _rerank_parents_by_crossencoder(self, query: str, parents: list[Document]):
        info = {
            "rerank_called": False,
            "rerank_ok": False,
            "rerank_error": None,
        ...
        try:
            ...
            info["rerank_ok"] = True
            info["rerank_scores"] = [scores[i] for i in picked_idx]
    ...
    def rerank_parents(self, query: str, parents: list[Document]) -> tuple[list[Document], dict[str, Any]]:
        if not self.rerank_enabled or self.rerank_backend == "none":
            return parents, {
                "rerank_called": False,
                "rerank_ok": False,
                "rerank_error": "disabled",
                "rerank_scores": [],
            }

        if self.rerank_backend == "crossencoder":
            return self._rerank_parents_by_crossencoder(query, parents)

        # 默认 qwen
        return self._rerank_parents_by_qwen(query, parents)
    ...
    def hybrid_search(
        ...
        parents, rr_info = self.rerank_parents(normalized_query, parents)
